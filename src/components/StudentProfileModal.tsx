  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-white border border-emerald-200 rounded-3xl overflow-hidden shadow-2xl z-10 my-auto flex flex-col max-h-[90vh] text-slate-900"
      >
        {/* Modal Header */}
        <div className="relative bg-emerald-900 text-white p-6 sm:p-8 shrink-0 border-b border-emerald-800">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 rounded-full bg-emerald-800 hover:bg-emerald-700 text-emerald-100 transition"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="text-center sm:text-left flex-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-emerald-800/80 text-emerald-200 text-[11px] font-bold border border-emerald-700 mb-2">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 text-xs text-emerald-100 font-medium">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 mt-4 pt-3 border-t border-emerald-800/80">
            ...
            <button
              onClick={handleShare}
              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-md shadow-emerald-950/20"
            >
        ...
        {updateFeedback && (
          <div className="bg-emerald-50 border-b border-emerald-200 p-3 px-6 text-xs text-emerald-900 flex items-center gap-2 font-medium">
        ...
        <div className="flex border-b border-emerald-100 bg-emerald-50/60 px-6 shrink-0">
          <button
          ...
          >
            <Award className="w-4 h-4 text-amber-500" /> Peer Awards Voting
          </button>
          ...
          >
            <MessageSquare className="w-4 h-4 text-emerald-600" /> Signatures ({visibleComments.length})
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {isEditing ? (
            <div className="bg-white p-5 rounded-2xl border border-emerald-200 shadow-sm space-y-4">
              <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                <Edit2 className="w-4 h-4 text-emerald-600" /> Edit Graduate Information
              </h3>

              {userSession?.role === 'student' && (
                <p className="text-[11px] text-amber-900 bg-amber-50 border border-amber-200 p-2.5 rounded-xl flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 font-mono"
          ...
          {activeTab === 'profile' && !isEditing && (
            <div className="space-y-5">
              {/* ADMIN FULL RECORD INSPECTION PANEL */}
              {userSession?.role === 'admin' && (
                <div className="bg-emerald-50 border border-emerald-300 p-4 sm:p-5 rounded-2xl space-y-3 text-xs text-emerald-950 shadow-sm">
                  <div className="flex items-center justify-between border-b border-emerald-200 pb-2">
                    <span className="font-black uppercase text-[11px] tracking-wider text-emerald-900 flex items-center gap-1.5">
                      <ShieldCheck className="w-4 h-4 text-emerald-700" /> Full Student Record (Admin View)
                    </span>
                    <span className="font-mono text-[10px] font-bold bg-emerald-200/80 text-emerald-950 px-2.5 py-0.5 rounded-full">
                      ID: {student.id}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 font-mono text-xs pt-1">
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Full Exam Number:</span>
                      <span className="font-black text-slate-900">{student.examNumber || student.id}</span>
                    </div>
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Date of Birth (DOB):</span>
                      <span className="font-black text-slate-900">{student.birthDate || 'Not specified'}</span>
                    </div>
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Photo File Asset:</span>
                      <span className="font-black text-slate-900 truncate block">{student.photoFilename || 'None'}</span>
                    </div>
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Student Email:</span>
                      <span className="font-black text-slate-900">{student.email || 'None registered'}</span>
                    </div>
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Student Phone:</span>
                      <span className="font-black text-slate-900">{student.phone || 'None registered'}</span>
                    </div>
                    <div className="bg-white p-2.5 rounded-xl border border-emerald-200">
                      <span className="text-slate-500 block text-[10px] font-sans font-bold">Featured Status:</span>
                      <span className="font-black text-emerald-800">{student.featuredOnHome ? 'Yes (Spotlight)' : 'No'}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Senior Quote Card */}
              <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 text-center relative">
                <p className="text-xs uppercase font-extrabold tracking-wider text-emerald-800 mb-2">Senior Quote</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="flex items-center gap-2 text-emerald-700 font-bold text-xs mb-1.5">
                    <Heart className="w-4 h-4 text-emerald-600" /> Hobbies & Passions
                  </div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="flex items-center gap-2 text-emerald-700 font-bold text-xs mb-1.5">
                    <Briefcase className="w-4 h-4 text-amber-500" /> Future Career Goal
                  </div>
                  <p className="text-slate-700 text-xs font-medium leading-relaxed">
                ...
                {student.email && (
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-2 text-emerald-700 font-bold text-xs mb-1.5">
                      <Mail className="w-4 h-4 text-emerald-600" /> Public Email
                    </div>
                    <a href={`mailto:${student.email}`} className="text-emerald-700 hover:underline text-xs font-mono">
                      {student.email}
                    </a>
                  </div>
                )}
                ...
                {student.phone && (
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-2 text-emerald-700 font-bold text-xs mb-1.5">
                      <Phone className="w-4 h-4 text-emerald-600" /> Public Phone
                    </div>
                    <a href={`tel:${student.phone}`} className="text-slate-700 hover:text-emerald-700 text-xs font-mono">
                      {student.phone}
                    </a>
                  </div>
                )}
            ...
            <div className="space-y-4">
              {!userSession && (
                <div className="bg-amber-50 border border-amber-200 p-3.5 rounded-2xl flex items-center justify-between gap-3 text-amber-900 text-xs">
                  ...
                  <button
                    onClick={onOpenAuth}
                    className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-bold transition shrink-0 flex items-center gap-1 shadow-sm"
                  >
          ...
          {activeTab === 'comments' && (
            <div className="space-y-4">
              {commentFeedback && (
                <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl text-amber-900 text-xs flex items-center gap-2 font-medium">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0" />
              ...
              {userSession ? (
                <form onSubmit={handleCommentSubmit} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                  <button
                    type="submit"
                    disabled={!commentText.trim()}
                    className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white font-bold text-xs rounded-2xl transition flex items-center gap-1.5 shrink-0 shadow-sm"
                  >
              ...
              <div className="space-y-3 pt-2">
                {visibleComments.length === 0 ? (
                  ...
                  visibleComments.map((comment) => (
                    <div
                      key={comment.id}
                      className="p-3.5 bg-white border border-slate-200 rounded-2xl flex items-start justify-between gap-3 shadow-sm"
                    >
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-bold text-xs text-slate-900">{comment.authorName}</span>
                          {comment.authorRole === 'admin' && (
                            <span className="px-1.5 py-0.2 bg-emerald-600 text-white font-black text-[9px] rounded-md">
                              ADMIN
                            </span>
                          )}
                          {comment.status === 'pending' && (
                            <span className="px-1.5 py-0.2 bg-amber-50 text-amber-800 border border-amber-200 font-bold text-[9px] rounded-md flex items-center gap-1">
