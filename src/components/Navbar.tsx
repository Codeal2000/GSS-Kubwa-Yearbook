...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-950/85 backdrop-blur-xl border-b border-slate-800/80 shadow-lg transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3 py-3">
          ...
          <div className="flex items-center justify-between w-full md:w-auto">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveTab('all')}>
              <div className="w-10 h-10 bg-indigo-600 group-hover:bg-indigo-500 rounded-xl flex items-center justify-center text-white shadow-md shadow-indigo-600/30 transition duration-200">
                <GraduationCap className="w-5 h-5" />
              </div>
              <div>
                <h1 className="text-lg sm:text-xl font-black text-white tracking-tight leading-none">
                  GSS KUBWA <span className="text-indigo-400">2026</span>
                </h1>
                <p className="text-[11px] font-semibold text-slate-400 mt-1 flex items-center gap-1">
                  Digital Yearbook • <span className="text-indigo-400 font-bold">{totalStudents} Graduates</span>
                </p>
              </div>
            </div>
            ...
            <div className="md:hidden flex items-center gap-2">
              {userSession?.role === 'admin' && (
                <button
                  ...
                  className="p-2 text-slate-200 bg-slate-900 border border-slate-800 rounded-lg text-xs font-bold transition"
                  title="Admin Panel"
                >
          ...
          <div className="w-full md:max-w-md relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-900/90 border border-slate-800 rounded-xl text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition"
            />
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            {userSession?.role === 'admin' && (
              <button
                onClick={onOpenAdminPanel}
                className="px-3 py-2 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-sm"
              >
            ...
            {userSession ? (
              <div className="flex items-center gap-2.5 bg-slate-900/90 p-1.5 pr-3 rounded-xl border border-slate-800">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] uppercase tracking-wider font-extrabold text-indigo-400">
              ...
              <button
                onClick={onOpenAuth}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-2 shadow-md shadow-indigo-600/20"
              >
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 overflow-x-auto py-2 border-t border-slate-800/80 no-scrollbar">
          <button
          ...
          >
            <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" /> Featured Spotlight
          </button>
